...
### GHCR Deployment (GitHub Actions)

This repo automatically publishes Docker images to:

- `ghcr.io/lysdestic/plex-collection-shuffler`

Publish triggers:

- Push to `main` -> publishes `:latest`
- Push to `home-assistant-webhook` -> publishes `:home-assistant-webhook`
- Branch/sha tags are also generated by the workflow metadata

Note: after the first publish, set package visibility in GHCR to public if you want unauthenticated pulls.

Compose snippet using the published image:

```yaml
services:
  plex-collection-shuffler:
    image: ghcr.io/lysdestic/plex-collection-shuffler:home-assistant-webhook
    container_name: plex-collection-shuffler
    restart: unless-stopped
    env_file:
      - .env
    environment:
      PLEX_WEBHOOK_HOST: 0.0.0.0
      PLEX_WEBHOOK_PORT: 8787
    ports:
      - "8787:8787"
```
